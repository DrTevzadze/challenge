...
function PartyBForm() {
  const settlements = useSelector((state: RootState) => state.settlement.settlements);
  ...
  return (
    <Modal>
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 flex flex-col gap-2">
        <div className="mb-4">
          <label className="block text-gray-700 text-xl font-bold mb-2" htmlFor="action">
            Action
          </label>
          <select
            id="action"
          ...
          <p className="text-gray-500 text-sm mt-1 italic">Make sure you select the correct action</p>
        </div>
        ...
        <div className="mb-4">
          <label className="block text-gray-700 text-xl font-bold mb-2" htmlFor="comment">
            Comments
          </label>
          <textarea
            id="comment"
